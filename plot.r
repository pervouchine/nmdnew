...
target  = compute_psi(read.delim(paste(dir,args[2],".M07.tsv", sep=""), col.names=c('id','inc','exc','ret')))
control = compute_psi(read.delim(paste(dir,args[3],".M07.tsv", sep=""), col.names=c('id','inc','exc','ret')))
...
df1$bin = cut_number(df1$log10expt, 10)
...
print(summary(m))
df2$SD = 10^m$coefficients[1]*df2$log10expt^m$coefficients[2]
...
df3 = df2 %>% group_by(gene.name) %>% slice(which.min(q))
...
md = data.frame(x=seq(0.5,6,0.01))
md$y = 10^m$coefficients[1]*(md$x^m$coefficients[2])
