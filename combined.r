...
fields=c('id','gene.name','deltaPSIc','p','q')

df1 = na.omit(read.delim(nmd))[,fields]
df2 = read.delim(self, col.names=c('KD','cell','id','gene.name','deltaPSI','deltaPSIc','z','p','q'))[,fields]
f<-function(x) {y=max(abs(x))*sign(x[1]);if(length(x)==2 & prod(x)<0){y=NA};y}
df2 = ddply(df2,.(id,gene.name),summarize, deltaPSIc=f(deltaPSIc), p=mean(p), q=mean(q))
df = merge(df1,df2, by=c('id','gene.name'),suffixes=c('.NMD','.SELF'))
